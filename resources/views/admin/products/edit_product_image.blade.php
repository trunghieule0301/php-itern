    ...
    <div class="col-lg-12">
        ...
        ?>
                <div class="position-center">
                    <form role="form" enctype="multipart/form-data" method="post" action="{{url('adminPage/updateProductImage/'.$edit_product->image_id)}}">
                        {{csrf_field()}}

                        <div class="form-group">
                            <label for="product">Product</label>
                            <select id="product" name="product_name" class="form-control input-sm m-bot15">
                                @foreach($img_product as $key => $product_value)
                                @if($product_value->product_id == $edit_product->image_product)
                                    <option selected value="{{$product_value->product_id}}">{{$key + 1}}. {{$product_value->product_name}}</option>
                                @else
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-group" >
                                ...
                                <div style="margin-top:10px;">
                                <embed type="{{$edit_product->image_mime}}" src='data:{{$edit_product->image_mime}};base64,{{base64_encode($edit_product->image_data)}}' width="50" />
                                </div>
                            </div>
                        </div>
                        
                        <button type="submit" name="add_brand" class="btn btn-info">Update</button>
                    </form>
                </div>
            </div>
            <script>
